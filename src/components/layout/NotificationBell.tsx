...
type Notif = { id: string, title: string, message: string, isRead: boolean, createdAt: Date }
...
export default function NotificationBell() {
  ...
  useEffect(() => {
    getMyNotifications().then(data => {
      // Safely handle if data is somehow invalid array
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <div key={n.id} style={{ 
                  ...
                  background: n.isRead ? 'transparent' : 'rgba(var(--primary), 0.05)',
                  cursor: n.isRead ? 'default' : 'pointer'
                }} onClick={(e) => handleRead(n.id, e)}>
                  <div style={{ fontSize: 13, fontWeight: n.isRead ? 500 : 700, color: 'var(--text-primary)', marginBottom: 4, display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                    <span>{n.title}</span>
                    {!n.isRead && <Check size={14} color="var(--primary)" style={{ flexShrink: 0 }} />}
                  </div>
